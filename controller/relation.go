...
func FriendList(c *gin.Context) {
	// 获取URl中的user_id参数   为待查询用户好友列表的ID
	friendsID, _ := strconv.ParseUint(c.Query("user_id"), 10, 64)
	// 判断是否登录
	var (
		isLogin  bool
		viewerID uint64
	...
	// 获取用户的好友列表
	friendList, err := service.GetFriendListByUserId(friendsID)
	if err != nil {
		c.JSON(http.StatusInternalServerError, Response{StatusCode: 1, StatusMsg: "get Friend list failed"})
		return
	}

	//生成response
	var ResultList []FriendUser
